...
BASE_REGISTRY_FILE = Path(__file__).resolve().parent / "data" / "contas_bancarias.xlsx"
...
def load_registry(files: list[InputFile]) -> tuple[list[dict], dict[str, str]]:
    ...
    if xlsx is not None:
        ...
        if not BASE_REGISTRY_FILE.exists():
            raise ReconciliationError(
                "Base de contas bancárias não encontrada. "
                "Inclua o arquivo mvp_app/data/contas_bancarias.xlsx ou envie a planilha XLSX junto com os PDFs."
            )
        wb = load_workbook(BASE_REGISTRY_FILE, read_only=True, data_only=True)
    ws = wb.active
    rows = []
    desc_map = {}
